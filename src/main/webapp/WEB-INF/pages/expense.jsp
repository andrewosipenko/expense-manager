...
<template:page expensesTabIsActive="${true}">
        ...
        <table class="table table-hover" style="margin-top: 30px">
            ...
            <tbody>
                <tr>
                    ...
                    <td><c:out value="${helper.amount} ${helper.currency.getCurrencyCode()}"/></td>
    ...
    <div class="jumbotron">
        ...
        <form method="post">
            <label>
                <input type="text" name="person" value="${isAddPage? '' : helper.person}"/>
                Person
            </label>
            <span class="pull-right">
                <label>
                    <input type="text" name="amount" value="${isAddPage? '' : helper.amount}" style=""/>
                    Amount
                </label>
                <select style="height:35px; width:70px; vertical-align: top" name="currency">
                    <c:forTokens var="code" items="USD, EUR, RUB, BYN, CNY" delims=", ">
                        <option value="${code}" ${(!isAddPage && helper.currency.getCurrencyCode().equals(code)) ? 'selected' : ''}><c:out value="${code}"/></option>
                    </c:forTokens>
                </select>
            </span>
            <label style="width:100%">
                <input type="text" name="description" value="${isAddPage? '' : helper.description}"/>
                Description
            </label><br/>
            <label>
                <input type="date" name="date" value="${isAddPage? '' : helper.date}"/>
                Date
            </label>
